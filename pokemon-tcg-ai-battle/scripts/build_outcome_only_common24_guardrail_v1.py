#!/usr/bin/env python3
"""Materialize the ATTACH+120 full-common24 evaluation guardrail."""
...
def _write_new(path: Path, value: object) -> None:
    ...
    try:
        ...
        try:
            ...
            raise FileExistsError(f"refusing to replace existing common24 guardrail: {path}") from exc
    finally:
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--broad-config", type=Path, required=True)
    parser.add_argument("--candidate-id", default="attach-plus-120")
    parser.add_argument("--action-deltas-json", default='{"ATTACH":120}')
    parser.add_argument("--base-seed", type=int, default=14910480)
    ...
    artifact = build_outcome_only_common24_guardrail_v1(
        ...
        broad_config_path=args.broad_config,
    ...
    print(json.dumps({"manifest": str(args.output.resolve()), "games": str(games_path.resolve()), "screen_sha256": manifest["screen_sha256"], "slot_count": 96, "execution_allowed": False}, sort_keys=True))
    return 0
