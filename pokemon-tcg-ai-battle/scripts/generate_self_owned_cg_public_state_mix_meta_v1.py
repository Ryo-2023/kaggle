#!/usr/bin/env python3
"""Generate a bounded official-data-only public-state source epoch.

The command creates independent scratch decks from the official card CSV,
renders the P1 public-state policy surface, and stages a local-only meta batch.
It deliberately stops before smoke, promotion, CEM, or submission.  Use the
explicit ``--execute`` flag; all generated manifests retain research-only
authority.
"""
...
from mage_ptcg.meta_specialist.cg_p1_public_state_mix_v1 import (  # noqa: E402
    BASE_SOURCE_SHA256,
...
PLAN_SCHEMA = "cg-p1-public-state-mix-plan-v1"
AUTHORITY_FALSE = {
...
class PublicStateMixPlanError(ValueError):
...
def _canonical_json(value: object) -> bytes:
    try:
        ...
        raise PublicStateMixPlanError("value is not canonical JSON") from exc
...
def _sha256_file(path: Path) -> str:
    if path.is_symlink() or not path.is_file():
        raise PublicStateMixPlanError(f"regular file required: {path}")
...
def _read_json(path: Path) -> Mapping[str, Any]:
    try:
        ...
        raise PublicStateMixPlanError(f"cannot read plan: {path}") from exc
    if not isinstance(value, Mapping):
        raise PublicStateMixPlanError("plan root must be an object")
    return value


def _resolve_repo_path(value: object, *, field: str) -> Path:
    if not isinstance(value, str) or not value.strip():
        raise PublicStateMixPlanError(f"{field} must be a non-empty path")
    path = Path(value)
...
def _identifier(value: object, field: str) -> str:
    if not isinstance(value, str) or not value or any(
        ...
        raise PublicStateMixPlanError(f"{field} must be an ASCII identifier")
    return value


def load_public_state_mix_plan_v1(path: str | Path) -> dict[str, object]:
    """Load and normalize one content-addressed source plan."""
    ...
    if set(raw) != expected or raw.get("schema_version") != PLAN_SCHEMA:
        raise PublicStateMixPlanError("plan schema or fields are invalid")
    for field in ("source_epoch", "seed_namespace"):
        if not isinstance(raw[field], str) or not raw[field].strip():
            raise PublicStateMixPlanError(f"{field} must be non-empty")
    card_database = _resolve_repo_path(raw["card_database"], field="card_database")
    if not card_database.is_file():
        raise PublicStateMixPlanError(f"card database is missing: {card_database}")

    scan_roots = raw["public_scan_roots"]
    if not isinstance(scan_roots, list) or not scan_roots:
        raise PublicStateMixPlanError("public_scan_roots must be non-empty")
    resolved_scan_roots: list[str] = []
    for index, value in enumerate(scan_roots):
        root = _resolve_repo_path(value, field=f"public_scan_roots[{index}]")
        if not root.is_dir():
            raise PublicStateMixPlanError(f"public scan root is missing: {root}")
        resolved_scan_roots.append(str(root))

    recipes = raw["deck_recipes"]
    if not isinstance(recipes, list) or not recipes:
        raise PublicStateMixPlanError("deck_recipes must be non-empty")
    ...
    for index, item in enumerate(recipes):
        if not isinstance(item, Mapping) or set(item) != {"id", "spec", "seed", "ordinal"}:
            raise PublicStateMixPlanError(f"deck_recipes[{index}] has invalid fields")
        recipe_id = _identifier(item["id"], f"deck_recipes[{index}].id")
        if recipe_id in recipes_by_id:
            raise PublicStateMixPlanError(f"duplicate deck recipe: {recipe_id}")
        if type(item["seed"]) is not int or type(item["ordinal"]) is not int or item["ordinal"] < 0:
            raise PublicStateMixPlanError(f"deck recipe {recipe_id} has invalid seed/ordinal")
        spec = _resolve_repo_path(item["spec"], field=f"deck_recipes[{index}].spec")
        if not spec.is_file():
            raise PublicStateMixPlanError(f"deck recipe spec is missing: {spec}")
    ...
    if not isinstance(variants, list) or not variants:
        raise PublicStateMixPlanError("policy_variants must be non-empty")
    variants_by_id: dict[str, dict[str, object]] = {}
    for index, item in enumerate(variants):
        if not isinstance(item, Mapping) or set(item) != {"id", "deck_recipe_id", "overrides"}:
            raise PublicStateMixPlanError(f"policy_variants[{index}] has invalid fields")
        variant_id = _identifier(item["id"], f"policy_variants[{index}].id")
        if variant_id in variants_by_id:
            raise PublicStateMixPlanError(f"duplicate policy variant: {variant_id}")
        recipe_id = _identifier(item["deck_recipe_id"], f"policy_variants[{index}].deck_recipe_id")
        if recipe_id not in recipes_by_id:
            raise PublicStateMixPlanError(f"variant references unknown deck recipe: {recipe_id}")
        if not isinstance(item["overrides"], Mapping):
            raise PublicStateMixPlanError(f"variant {variant_id} overrides must be an object")
        try:
            config = PublicStateMixConfig.from_mapping(dict(item["overrides"]))
        except (TypeError, ValueError) as exc:
            raise PublicStateMixPlanError(f"variant {variant_id} has invalid overrides") from exc
    ...
    return {
        ...
        "public_scan_roots": tuple(resolved_scan_roots),
...
def run_generation_v1(
    *,
    ...
    """Generate and stage all packages named by a source plan."""

    plan_data = load_public_state_mix_plan_v1(plan)
    ...
    if _sha256_file(source_main) != BASE_SOURCE_SHA256 or not source_cg.is_dir():
        raise PublicStateMixPlanError("p1 source package is not the immutable P1 runtime")
    ...
    recipe_by_id = {str(item["id"]): item for item in plan_data["deck_recipes"]}
    ...
    for variant in plan_data["policy_variants"]:
        ...
        config = PublicStateMixConfig.from_mapping(dict(variant["config"]))
        candidate_id = f"self-owned-cg-public-state-mix-{variant_id}-{config.config_sha256()[:12]}"
        package_root = package_roots / variant_id
        package_manifest = materialize_public_state_mix_package(
            source_package=source_package,
        ...
        if policy_sha in seen_policies:
            raise PublicStateMixPlanError(f"policy collision: {variant_id}")
        seen_policies.add(policy_sha)
        deck_result = deck_results[str(recipe["id"])]
        body: dict[str, object] = {
            "schema_version": "cg-p1-public-state-mix-source-v1",
            "status": "COMPLETE",
            ...
            "deck_candidate_id": deck_result["candidate_id"],
        ...
        _write_canonical(generation_path, body)
    ...
    overall["manifest_sha256"] = _sha256_bytes(_canonical_json(overall))
    manifest_path = output_root / "source_epoch_manifest.json"
    _write_canonical(manifest_path, overall)
    return {
        "status": "STAGED",
        "output_root": str(output_root),
        "staged_root": str(staged_root),
        "source_epoch_manifest": str(manifest_path),
        "source_count": len(identities),
        "policy_count": len(seen_policies),
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        result = run_generation_v1(
            plan=args.plan,
            output=args.output,
            p1_source_package=args.p1_source_package,
        )
    except (PublicStateMixPlanError, OSError, ValueError) as exc:
