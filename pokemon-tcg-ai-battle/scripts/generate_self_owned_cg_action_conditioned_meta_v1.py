#!/usr/bin/env python3
"""Generate an official-card-only action-conditioned CG source epoch.

The command only stages research-only packages.  Runtime smoke, promotion,
split sealing, and CABT performance evaluation are separate explicit gates.
"""
...
PLAN_SCHEMA = "self-owned-cg-action-conditioned-factorial-plan-v1"
AUTHORITY_FALSE = {
...
class ActionConditionedPlanError(ValueError):
...
def _canonical_json(value: object) -> bytes:
    try:
        ...
        raise ActionConditionedPlanError("value is not canonical JSON") from exc
...
def _sha256_file(path: Path) -> str:
    if path.is_symlink() or not path.is_file():
        raise ActionConditionedPlanError(f"regular file required: {path}")
...
def _read_json(path: Path) -> Mapping[str, Any]:
    try:
        ...
        raise ActionConditionedPlanError(f"cannot read plan: {path}") from exc
    if not isinstance(value, Mapping):
        raise ActionConditionedPlanError("plan root must be an object")
    return value


def _resolve_repo_path(value: object, *, field: str) -> Path:
    if not isinstance(value, str) or not value.strip():
        raise ActionConditionedPlanError(f"{field} must be a non-empty path")
    path = Path(value)
...
def _identifier(value: object, field: str) -> str:
    if not isinstance(value, str) or not value or any(
        ...
        raise ActionConditionedPlanError(f"{field} must be an ASCII identifier")
    return value


def load_action_conditioned_plan_v1(path: str | Path) -> dict[str, object]:
    """Load and normalize one content-addressed action-conditioned plan."""
    ...
    if set(raw) != expected or raw.get("schema_version") != PLAN_SCHEMA:
        raise ActionConditionedPlanError("plan schema or fields are invalid")
    for field in ("source_epoch", "seed_namespace"):
        if not isinstance(raw[field], str) or not raw[field].strip():
            raise ActionConditionedPlanError(f"{field} must be non-empty")
    card_database = _resolve_repo_path(raw["card_database"], field="card_database")
    if not card_database.is_file():
        raise ActionConditionedPlanError(f"card database is missing: {card_database}")
    roots = raw["public_scan_roots"]
    if not isinstance(roots, list) or not roots:
        raise ActionConditionedPlanError("public_scan_roots must be non-empty")
    scan_roots: list[str] = []
    for index, value in enumerate(roots):
        root = _resolve_repo_path(value, field=f"public_scan_roots[{index}]")
        if not root.is_dir():
            raise ActionConditionedPlanError(f"public scan root is missing: {root}")
        scan_roots.append(str(root))

    recipes_raw = raw["deck_recipes"]
    if not isinstance(recipes_raw, list) or not recipes_raw:
        raise ActionConditionedPlanError("deck_recipes must be non-empty")
    recipes: dict[str, dict[str, object]] = {}
    for index, item in enumerate(recipes_raw):
        if not isinstance(item, Mapping) or set(item) != {"id", "spec", "seed", "ordinal"}:
            raise ActionConditionedPlanError(f"deck_recipes[{index}] has invalid fields")
        recipe_id = _identifier(item["id"], f"deck_recipes[{index}].id")
        if recipe_id in recipes:
            raise ActionConditionedPlanError(f"duplicate deck recipe: {recipe_id}")
        if type(item["seed"]) is not int or type(item["ordinal"]) is not int or item["ordinal"] < 0:
            raise ActionConditionedPlanError(f"deck recipe {recipe_id} has invalid seed/ordinal")
        spec = _resolve_repo_path(item["spec"], field=f"deck_recipes[{index}].spec")
        if not spec.is_file():
            raise ActionConditionedPlanError(f"deck recipe spec is missing: {spec}")
        recipes[recipe_id] = {"id": recipe_id, "spec": str(spec), "seed": int(item["seed"]), "ordinal": int(item["ordinal"])}

    variants_raw = raw["policy_variants"]
    if not isinstance(variants_raw, list) or not variants_raw:
        raise ActionConditionedPlanError("policy_variants must be non-empty")
    variants: dict[str, dict[str, object]] = {}
    for index, item in enumerate(variants_raw):
        if not isinstance(item, Mapping) or set(item) != {"id", "deck_recipe_id", "overrides"}:
            raise ActionConditionedPlanError(f"policy_variants[{index}] has invalid fields")
        variant_id = _identifier(item["id"], f"policy_variants[{index}].id")
        if variant_id in variants:
            raise ActionConditionedPlanError(f"duplicate policy variant: {variant_id}")
        recipe_id = _identifier(item["deck_recipe_id"], f"policy_variants[{index}].deck_recipe_id")
        if recipe_id not in recipes:
            raise ActionConditionedPlanError(f"variant references unknown deck recipe: {recipe_id}")
        overrides = item["overrides"]
        if not isinstance(overrides, Mapping):
            raise ActionConditionedPlanError(f"variant {variant_id} overrides must be an object")
        try:
            config = ActionConditionedConfig.from_mapping(dict(overrides))
        except (TypeError, ValueError) as exc:
            raise ActionConditionedPlanError(f"variant {variant_id} has invalid overrides") from exc
        variants[variant_id] = {
            "id": variant_id,
...
def run_generation_v1(*, plan: str | Path, output: str | Path, p1_source_package: str | Path) -> dict[str, object]:
    plan_data = load_action_conditioned_plan_v1(plan)
    ...
    if _sha256_file(source_package / "main.py") != BASE_SOURCE_SHA256 or not (source_package / "cg").is_dir():
        raise ActionConditionedPlanError("P1 source package is not the immutable runtime")
    ...
    for variant in plan_data["policy_variants"]:
        ...
        if deck_hash in seen_decks:
            raise ActionConditionedPlanError(f"factorial deck collision: {variant_id}")
        seen_decks.add(deck_hash)
        config = ActionConditionedConfig.from_mapping(dict(variant["config"]))
        candidate_id = f"self-owned-cg-action-conditioned-{variant_id}-{config.config_sha256()[:12]}"
        package = package_roots / variant_id
        package_manifest = materialize_action_conditioned_package(
            source_package=source_package,
        ...
        if policy_hash in seen_policies:
            raise ActionConditionedPlanError(f"factorial policy collision: {variant_id}")
        seen_policies.add(policy_hash)
        body: dict[str, object] = {
            "schema_version": "self-owned-cg-action-conditioned-source-v1",
            "status": "COMPLETE",
    ...
    manifest_path = output_root / "action_conditioned_manifest.json"
    _write_canonical(manifest_path, epoch)
    return {
        "status": "STAGED",
        "output_root": str(output_root),
        "staged_root": str(output_root / "staged"),
        "action_conditioned_manifest": str(manifest_path),
        "source_count": len(identities),
        "deck_count": len(seen_decks),
        "policy_count": len(seen_policies),
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    except (ActionConditionedPlanError, FileExistsError, OSError, ValueError) as exc:
