#!/usr/bin/env python3
"""Stage an official-card-only deck-adaptive CG source epoch.

This runner generates scratch decks from the official card CSV, renders an
independent public-state policy for each deck, and stops at a staged batch.
Runtime smoke, promotion, CEM, and submission remain separate explicit gates.
"""
...
from mage_ptcg.meta_specialist.cg_deck_adaptive_renderer_v1 import (  # noqa: E402
    DeckAdaptiveConfig,
    RENDERER_FAMILY,
...
PLAN_SCHEMA = "self-owned-cg-deck-adaptive-factorial-plan-v1"
SOURCE_KIND = "self_owned_official_card_data_deck_with_deck_adaptive_policy"
AUTHORITY_FALSE = {
...
class DeckAdaptivePlanError(ValueError):
    """Raised when a deck-adaptive source plan is malformed or unsafe."""
...
def _canonical_json(value: object) -> bytes:
    try:
        ...
        raise DeckAdaptivePlanError("value is not canonical JSON") from exc
...
def _sha256_file(path: Path) -> str:
    if path.is_symlink() or not path.is_file():
        raise DeckAdaptivePlanError(f"regular file required: {path}")
...
def _read_json(path: Path) -> Mapping[str, Any]:
    try:
        ...
        raise DeckAdaptivePlanError(f"cannot read plan: {path}") from exc
    if not isinstance(value, Mapping):
        raise DeckAdaptivePlanError("plan root must be an object")
    return value


def _resolve_repo_path(value: object, *, field: str) -> Path:
    if not isinstance(value, str) or not value.strip():
        raise DeckAdaptivePlanError(f"{field} must be a non-empty path")
    path = Path(value)
...
def _identifier(value: object, field: str) -> str:
    if not isinstance(value, str) or not value or any(
        ...
        raise DeckAdaptivePlanError(f"{field} must be an ASCII identifier")
    return value


def load_deck_adaptive_plan_v1(path: str | Path) -> dict[str, object]:
    """Load and normalize one content-addressed deck-adaptive plan."""
    ...
    if not isinstance(variants_raw, list) or not variants_raw:
        raise DeckAdaptivePlanError("policy_variants must be non-empty")
    variants: dict[str, dict[str, object]] = {}
    for index, item in enumerate(variants_raw):
        if not isinstance(item, Mapping) or set(item) != {"id", "deck_recipe_id", "overrides"}:
            raise DeckAdaptivePlanError(f"policy_variants[{index}] has invalid fields")
        variant_id = _identifier(item["id"], f"policy_variants[{index}].id")
        if variant_id in variants:
            raise DeckAdaptivePlanError(f"duplicate policy variant: {variant_id}")
        recipe_id = _identifier(item["deck_recipe_id"], f"policy_variants[{index}].deck_recipe_id")
        if recipe_id not in recipes:
            raise DeckAdaptivePlanError(f"variant references unknown deck recipe: {recipe_id}")
        overrides = item["overrides"]
        if not isinstance(overrides, Mapping):
            raise DeckAdaptivePlanError(f"variant {variant_id} overrides must be an object")
        try:
            config = DeckAdaptiveConfig.from_mapping(dict(overrides))
        except (TypeError, ValueError) as exc:
            raise DeckAdaptivePlanError(f"variant {variant_id} has invalid overrides") from exc
        variants[variant_id] = {
            "id": variant_id,
    ...
    if len(variants) != len(recipes):
        raise DeckAdaptivePlanError("each deck recipe must map to exactly one policy variant")
    return {
        "path": str(plan_path),
...
def run_generation_v1(
    *, plan: str | Path, output: str | Path, runtime_package: str | Path
) -> dict[str, object]:
    plan_data = load_deck_adaptive_plan_v1(plan)
    output_root = Path(output).resolve()
    if output_root.exists():
        raise FileExistsError(f"output root already exists: {output_root}")
    runtime = Path(runtime_package).resolve()
    if not (runtime / "cg").is_dir():
        raise DeckAdaptivePlanError("runtime package must contain cg/")
    ...
    for variant in plan_data["policy_variants"]:
        ...
        if deck_hash in seen_decks:
            raise DeckAdaptivePlanError(f"deck collision: {variant_id}")
        seen_decks.add(deck_hash)
        config = DeckAdaptiveConfig.from_mapping(dict(variant["config"]))
        candidate_id = candidate_id_for_config(
            config, generation=0, index=len(packages), deck_sha256=deck_hash
        )
        package = package_roots / variant_id
        package_manifest = materialize_deck_adaptive_package(
            source_runtime_package=runtime,
            deck_package=deck_roots / str(recipe["id"]) / "package",
        ...
        if policy_hash in seen_policies:
            raise DeckAdaptivePlanError(f"policy collision: {variant_id}")
        seen_policies.add(policy_hash)
        body: dict[str, object] = {
            "schema_version": "self-owned-cg-deck-adaptive-source-v1",
            "status": "COMPLETE",
    ...
    manifest_path = output_root / "deck_adaptive_manifest.json"
    _write_canonical(manifest_path, epoch)
    return {
        "status": "STAGED",
        "output_root": str(output_root),
        "staged_root": str(output_root / "staged"),
        "deck_adaptive_manifest": str(manifest_path),
        "source_count": len(identities),
        "deck_count": len(seen_decks),
        "policy_count": len(seen_policies),
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if not args.execute:
        ...
        return 2
    try:
        result = run_generation_v1(plan=args.plan, output=args.output, runtime_package=args.runtime_package)
    except (DeckAdaptivePlanError, FileExistsError, OSError, ValueError) as exc:
