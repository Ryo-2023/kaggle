#!/usr/bin/env python3
"""Seal a split-balanced, research-only behavior meta source pool."""
...
def _read_spec(path: Path) -> list[Mapping[str, object]]:
    ...
    if not isinstance(entries, list):
        raise StratifiedBehaviorMetaError(
            "spec must be a JSON list or an object with an entries list"
        )
    return entries
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        result = seal_stratified_behavior_meta_v2(
    ...
    except (StratifiedBehaviorMetaError, FileExistsError, OSError, ValueError, json.JSONDecodeError) as exc:
