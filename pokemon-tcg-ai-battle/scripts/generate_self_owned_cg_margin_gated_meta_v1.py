#!/usr/bin/env python3
"""Stage an official-card-only margin-gated CG source epoch.

The source policy is rendered from the sealed P1 package and bound to a fresh
self-owned deck generated from the official card CSV.  This command stops at
research-only staging; runtime smoke, promotion, split sealing, and CABT
performance gates remain separate commands.
"""
...
MARGIN_GATED_PLAN_SCHEMA = "self-owned-cg-margin-gated-factorial-plan-v1"
GENERATION_SCHEMA = "self-owned-cg-margin-gated-source-v1"
SOURCE_KIND = "self_owned_official_card_data_deck_with_margin_gated_public_state_policy"
AUTHORITY_FALSE = {
...
class MarginGatedPlanError(ValueError):
...
def _canonical_json(value: object) -> bytes:
    try:
        ...
        raise MarginGatedPlanError("value is not canonical JSON") from exc
...
def _sha256_file(path: Path) -> str:
    if path.is_symlink() or not path.is_file():
        raise MarginGatedPlanError(f"regular file required: {path}")
...
def _read_json(path: Path) -> Mapping[str, Any]:
    try:
        ...
        raise MarginGatedPlanError(f"cannot read plan: {path}") from exc
    if not isinstance(value, Mapping):
        raise MarginGatedPlanError("plan root must be an object")
    return value


def _resolve_repo_path(value: object, *, field: str) -> Path:
    if not isinstance(value, str) or not value.strip():
        raise MarginGatedPlanError(f"{field} must be a non-empty path")
    path = Path(value)
...
def _identifier(value: object, field: str) -> str:
    if not isinstance(value, str) or not value or any(
        ...
        raise MarginGatedPlanError(f"{field} must be an ASCII identifier")
    return value


def load_margin_gated_plan_v1(path: str | Path) -> dict[str, object]:
    """Load and normalize one content-addressed margin-gated plan."""
    ...
    if set(raw) != expected or raw.get("schema_version") != MARGIN_GATED_PLAN_SCHEMA:
        raise MarginGatedPlanError("plan schema or fields are invalid")
    for field in ("source_epoch", "seed_namespace"):
        if not isinstance(raw[field], str) or not raw[field].strip():
            raise MarginGatedPlanError(f"{field} must be non-empty")
    card_database = _resolve_repo_path(raw["card_database"], field="card_database")
    source_package = _resolve_repo_path(raw["p1_source_package"], field="p1_source_package")
    if not card_database.is_file():
        raise MarginGatedPlanError(f"card database is missing: {card_database}")
    if not source_package.is_dir():
        raise MarginGatedPlanError(f"P1 source package is missing: {source_package}")

    roots = raw["public_scan_roots"]
    if not isinstance(roots, list) or not roots:
        raise MarginGatedPlanError("public_scan_roots must be non-empty")
    scan_roots: list[str] = []
    for index, value in enumerate(roots):
        root = _resolve_repo_path(value, field=f"public_scan_roots[{index}]")
        if not root.is_dir():
            raise MarginGatedPlanError(f"public scan root is missing: {root}")
        scan_roots.append(str(root))

    recipes_raw = raw["deck_recipes"]
    if not isinstance(recipes_raw, list) or not recipes_raw:
        raise MarginGatedPlanError("deck_recipes must be non-empty")
    recipes: dict[str, dict[str, object]] = {}
    for index, item in enumerate(recipes_raw):
        if not isinstance(item, Mapping) or set(item) != {"id", "spec", "seed", "ordinal"}:
            raise MarginGatedPlanError(f"deck_recipes[{index}] has invalid fields")
        recipe_id = _identifier(item["id"], f"deck_recipes[{index}].id")
        if recipe_id in recipes:
            raise MarginGatedPlanError(f"duplicate deck recipe: {recipe_id}")
        if type(item["seed"]) is not int or type(item["ordinal"]) is not int or item["ordinal"] < 0:
            raise MarginGatedPlanError(f"deck recipe {recipe_id} has invalid seed/ordinal")
        spec = _resolve_repo_path(item["spec"], field=f"deck_recipes[{index}].spec")
        if not spec.is_file():
            raise MarginGatedPlanError(f"deck recipe spec is missing: {spec}")
        recipes[recipe_id] = {
            "id": recipe_id,
            "spec": str(spec),
            "seed": int(item["seed"]),
    ...
    if not isinstance(variants_raw, list) or not variants_raw:
        raise MarginGatedPlanError("policy_variants must be non-empty")
    variants: dict[str, dict[str, object]] = {}
    for index, item in enumerate(variants_raw):
        if not isinstance(item, Mapping) or set(item) != {"id", "deck_recipe_id", "overrides"}:
            raise MarginGatedPlanError(f"policy_variants[{index}] has invalid fields")
        ...
        if variant_id in variants:
            raise MarginGatedPlanError(f"duplicate policy variant: {variant_id}")
        if recipe_id not in recipes:
            raise MarginGatedPlanError(f"variant references unknown deck recipe: {recipe_id}")
        overrides = item["overrides"]
        if not isinstance(overrides, Mapping):
            raise MarginGatedPlanError(f"variant {variant_id} overrides must be an object")
        try:
            config = MarginGatedConfig.from_mapping(dict(overrides))
        except (TypeError, ValueError) as exc:
            raise MarginGatedPlanError(f"variant {variant_id} has invalid overrides") from exc
        variants[variant_id] = {
            "id": variant_id,
    ...
    return {
        ...
        "schema_version": MARGIN_GATED_PLAN_SCHEMA,
...
def run_generation_v1(*, plan: str | Path, output: str | Path) -> dict[str, object]:
    plan_data = load_margin_gated_plan_v1(plan)
    ...
    if _sha256_file(source_package / "main.py") != BASE_SOURCE_SHA256 or not (source_package / "cg").is_dir():
        raise MarginGatedPlanError("P1 source package is not the immutable runtime")
    ...
    for variant in plan_data["policy_variants"]:
        ...
        if deck_hash in seen_decks:
            raise MarginGatedPlanError(f"factorial deck collision: {variant_id}")
        seen_decks.add(deck_hash)
        config = MarginGatedConfig.from_mapping(dict(variant["config"]))
        candidate_id = f"self-owned-cg-margin-gated-{variant_id}-{config.config_sha256()[:12]}"
        package = package_roots / variant_id
        package_manifest = materialize_margin_gated_package(
            source_package=source_package,
            self_owned_deck_package=deck_roots / recipe_id / "package",
        ...
        if policy_hash in seen_policies:
            raise MarginGatedPlanError(f"factorial policy collision: {variant_id}")
    ...
    manifest_path = output_root / "margin_gated_manifest.json"
    _write_canonical(manifest_path, epoch)
    return {
        "status": "STAGED",
        "output_root": str(output_root),
        "staged_root": str(output_root / "staged"),
        "margin_gated_manifest": str(manifest_path),
        "source_count": len(identities),
        "deck_count": len(seen_decks),
        "policy_count": len(seen_policies),
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    except (MarginGatedPlanError, FileExistsError, OSError, ValueError) as exc:
