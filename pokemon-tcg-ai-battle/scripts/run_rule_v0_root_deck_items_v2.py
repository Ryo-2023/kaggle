#!/usr/bin/env python3
"""Sealed research-only Item surface for the P0 Rule v0/root deck lane.

This thin wrapper reuses the fail-closed runner while sealing a new, disjoint
candidate surface.  It never edits the production entrypoint or prior roots.
"""
...
base.SCHEMA = "meta-specialist-rule-v0-root-deck-items-v2"
base.OUTPUT_DEFAULT = base.ROOT / "runs/final-sprint-autonomous/rule-v0-root-deck-items-v2-weighted48-20260814"
base.WEIGHTED_BASE_SEED = 23_450_000
base.SURFACES = (
    ("root-premium-power-to-pokegear", 1141, 1122),
    ("root-switch-to-ultra-ball", 1123, 1121),
)
COMMON24_BASE_SEED = 23_460_000


def execute_common24(*, source_root: Path, output: Path) -> dict[str, object]:
    """Run broad common24 for every positive weighted candidate."""
    ...
    if len(references) != 24 or len(set(references)) != 24:
        raise ValueError("common24 config must contain exactly 24 IDs")
    by_id = {str(row["candidate_id"]): row for row in source_manifest["candidates"]}
    parent = source_manifest["parent"]
    manifest = {
        "schema_version": f"{base.SCHEMA}-common24",
        "purpose": "P0_RULE_V0_ROOT_DECK_ITEMS_COMMON24_GUARDRAIL",
        ...
        "parent": parent,
        "candidates": [by_id[str(row["candidate_id"])] for row in positives],
        ...
        "protocol": {"games_per_arm": 96, "games_per_seat": 2, "base_seed": COMMON24_BASE_SEED, "same_seed_schedule": True, "workers": 12, "worker_recycle_games": 16, "heldout_exposure": False},
    ...
    specs.extend((str(row["candidate_id"]), Path(str(row["deck_path"])), str(row["deck_file_sha256"])) for row in manifest["candidates"])
    summaries: dict[str, object] = {}
    arm_roots: dict[str, str] = {}
    all_ids: list[str] = []
    started = time.monotonic()
    for arm, deck_path, deck_sha in specs:
        games = base._build_arm_games(deck_path=deck_path, deck_sha=deck_sha, deck_id=arm, block_id=f"{base.SCHEMA}-common24-96-{arm}", references=references, base_seed=COMMON24_BASE_SEED, games_per_seat=2)
        games = tuple(replace(game, metadata={**dict(game.metadata), "common24_evaluation_only": True, "common24_config_sha256": base._file_sha(base.COMMON24_CONFIG), "weighted_subset_sha256": source_manifest["meta_train_subset"]["subset_sha256"]}) for game in games)
    ...
    for row in manifest["candidates"]:
        ...
        candidate_results.append({"candidate_id": arm, "deck_file_sha256": row["deck_file_sha256"], "deck_multiset_sha256": row["deck_multiset_sha256"], "score_rate": score, "delta_points": (score - parent_score) * 100.0, "fault_gate": int(summaries[arm]["faults"]) == 0, "status": "common24_positive_candidate_only" if int(summaries[arm]["faults"]) == 0 and score > parent_score else "candidate_only", "root": arm_roots[arm]})
    payload = {
    ...
    lines = ["# Rule v0/root deck items common24", "", f"- parent: {summaries['parent']['wins']}-{summaries['parent']['draws']}-{summaries['parent']['losses']}-{summaries['parent']['faults']} ({parent_score:.6f})"]
    lines.extend(f"- {row['candidate_id']}: {summaries[row['candidate_id']]['wins']}-{summaries[row['candidate_id']]['draws']}-{summaries[row['candidate_id']]['losses']}-{summaries[row['candidate_id']]['faults']} ({row['score_rate']:.6f}), delta={row['delta_points']:+.3f}pt" for row in candidate_results)
    ...
    final = {"schema_version": f"{base.SCHEMA}-common24", "output_root": str(output), "common24_manifest_sha256": manifest_sha, "summary_sha256": summary_sha, "summary_md_sha256": md_sha, "candidates": candidate_results, "all_faults_zero": payload["all_faults_zero"], "authority": dict(base.AUTHORITY_FALSE), "candidate_status": "candidate_only", "performance_run_started": True, "arm_roots": arm_roots}
    base._write_no_clobber(output / "final_summary.json", final)
    return final
...
def main() -> int:
    ...
    if args.common24_from_weighted:
        result = execute_common24(source_root=args.output.resolve(), output=(args.common24_output or Path(str(args.output) + "-common24")).resolve())
    else:
