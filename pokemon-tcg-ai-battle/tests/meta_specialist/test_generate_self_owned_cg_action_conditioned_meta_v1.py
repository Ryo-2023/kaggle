...
from scripts.generate_self_owned_cg_action_conditioned_meta_v1 import (
    PLAN_SCHEMA,
    load_action_conditioned_plan_v1,
)
...
def _write_plan(path: Path, *, overrides: dict[str, object] | None = None) -> None:
    payload = {
        ...
        "source_epoch": "cg-action-conditioned-test-20260816",
        "seed_namespace": "cg-action-conditioned-test-seed-v1",
        ...
        "policy_variants": [
            {
                ...
                "overrides": overrides or {"attack_lethal_behind_bonus": 12000},
            }
        ],
    }
...
def test_plan_loader_normalizes_config_and_paths(tmp_path: Path) -> None:
    ...
    _write_plan(path)
    plan = load_action_conditioned_plan_v1(path)
    assert plan["schema_version"] == PLAN_SCHEMA
    assert plan["policy_variants"][0]["config"]["attack_lethal_behind_bonus"] == 12000
    assert Path(plan["card_database"]).is_file()


def test_plan_loader_rejects_invalid_override(tmp_path: Path) -> None:
    path = tmp_path / "plan.json"
    _write_plan(path, overrides={"attack_lethal_behind_bonus": 999999})
    with pytest.raises(ValueError, match="invalid overrides"):
        load_action_conditioned_plan_v1(path)
