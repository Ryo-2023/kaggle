...
from mage_ptcg.meta_specialist.cg_p1_seat_conditioned_renderer_v1 import SeatConditionedConfig
from mage_ptcg.meta_specialist.cg_seat_conditioned_cem_v1 import (
    rank_valid_results,
    sample_population,
    update_distribution,
)
...
def test_rank_and_update_use_fault_free_elites() -> None:
    center = SeatConditionedConfig.default()
    positive = SeatConditionedConfig(seat0_attack_bonus=2000)
    negative = SeatConditionedConfig(seat0_attack_bonus=-2000)
    results = [
    ...
    assert next_center.seat0_attack_bonus == 1000
    assert scales["seat0_attack_bonus"] > 0


def test_rank_requires_enough_valid_candidates() -> None:
    with pytest.raises(ValueError, match="not enough valid candidates"):
        rank_valid_results(
            [{"config": SeatConditionedConfig.default(), "objective": 0.0, "faults": 1}],
            elite_count=1,
        )
