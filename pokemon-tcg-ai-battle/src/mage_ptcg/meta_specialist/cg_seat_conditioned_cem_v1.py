"""Dependency-free CEM helpers for the public-seat-conditioned P1 surface."""
...
from .cg_p1_seat_conditioned_renderer_v1 import PARAMETER_BOUNDS, SeatConditionedConfig


SCHEMA = "cg-seat-conditioned-cem-state-v1"
...
def sample_population(
    center: SeatConditionedConfig,
    *,
    generation: int,
    population_size: int = 8,
    seed: int = 20260816,
    scales: Mapping[str, float] | None = None,
) -> tuple[SeatConditionedConfig, ...]:
    ...
    while len(values) < population_size:
        ...
        values.append(SeatConditionedConfig.from_mapping(sampled))
    return tuple(values)
...
def rank_valid_results(
    ...
    for raw in results:
        ...
        if isinstance(config_raw, SeatConditionedConfig):
            config = config_raw
        elif isinstance(config_raw, Mapping):
            try:
                config = SeatConditionedConfig.from_mapping(config_raw)
            except ValueError:
                continue
        else:
            continue
...
def update_distribution(
...
) -> tuple[SeatConditionedConfig, dict[str, float]]:
    """Update mean and standard-deviation scales from elite configs."""

    center.validate()
    if not elites:
        raise ValueError("elites cannot be empty")
    configs: list[SeatConditionedConfig] = []
    for item in elites:
        value = item.get("config")
        config = value if isinstance(value, SeatConditionedConfig) else SeatConditionedConfig.from_mapping(value)
    ...
    return SeatConditionedConfig.from_mapping(updated), scales
