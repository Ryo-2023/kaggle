"""Hash-bound Lunar Cycle ability variants for the fixed P1 cg policy.

These research-only overlays add a narrow score bonus to Lunatone's
``Lunar Cycle`` ability when Solrock is visible and the actor's own hand is
small.  The base P1 source and all unsupported observations remain unchanged.
"""
...
_PATCHES: dict[str, str] = {
    "cg-p1-lunar-cycle-lowhand3-v1": r'''

# RESEARCH_VARIANT: cg-p1-lunar-cycle-lowhand3-v1
# Prefer LUNATONE's Lunar Cycle when Solrock is in play and hand_count <= 3.
_CG_P1_ABILITY_BASE_MAIN_SCORE = _main_score

def _main_score(obs, option: object) -> int:
    try:
        score = int(_CG_P1_ABILITY_BASE_MAIN_SCORE(obs, option))
        if getattr(option, "type", None) != OptionType.ABILITY:
            return score
        if _card_for_option(obs, option) is None or getattr(_card_for_option(obs, option), "id", None) != LUNATONE:
            return score
        hand = getattr(_mine(obs), "hand", None)
        hand_count = len(hand) if isinstance(hand, list) else None
        if _has(obs, SOLROCK) and isinstance(hand_count, int) and hand_count <= 3:
            return score + 12000
        return score
    except Exception:
        return 1000

_CG_P1_ABILITY_BASE_AGENT = agent

def agent(obs_dict: dict) -> list[int]:
    return _CG_P1_ABILITY_BASE_AGENT(obs_dict)
''',
    "cg-p1-lunar-cycle-lowhand4-v1": r'''

# RESEARCH_VARIANT: cg-p1-lunar-cycle-lowhand4-v1
# Prefer LUNATONE's Lunar Cycle when Solrock is in play and hand_count <= 4.
_CG_P1_ABILITY_BASE_MAIN_SCORE = _main_score
...
def _main_score(obs, option: object) -> int:
    try:
        ...
        if target is None or getattr(target, "id", None) != LUNATONE:
            return score
        hand = getattr(_mine(obs), "hand", None)
        hand_count = len(hand) if isinstance(hand, list) else None
        if _has(obs, SOLROCK) and isinstance(hand_count, int) and hand_count <= 4:
            return score + 8000
        return score
    except Exception:
        return 1000
...
def agent(obs_dict: dict) -> list[int]:
    return _CG_P1_ABILITY_BASE_AGENT(obs_dict)
''',
    "cg-p1-lunar-cycle-lowhand5-v1": r'''

# RESEARCH_VARIANT: cg-p1-lunar-cycle-lowhand5-v1
# Prefer LUNATONE's Lunar Cycle when Solrock is in play and hand_count <= 5.
_CG_P1_ABILITY_BASE_MAIN_SCORE = _main_score
...
def _main_score(obs, option: object) -> int:
    try:
        ...
        hand = getattr(_mine(obs), "hand", None)
        hand_count = len(hand) if isinstance(hand, list) else None
        if _has(obs, SOLROCK) and isinstance(hand_count, int) and hand_count <= 5:
            return score + 6000
        return score
    except Exception:
        return 1000
...
def render_p1_variant_source_v5(candidate_id: str) -> str:
...
def materialize_p1_variant_package_v5(
    *,
    ...
    rendered = render_p1_variant_source_v5(candidate_id).encode("utf-8")
    (target / "main.py").write_bytes(rendered)
    return {
        "schema_version": "cg-p1-policy-candidate-v5",
...
__all__ = [
    ...
    "BASE_SOURCE_SHA256",
    "VARIANT_IDS",
    "materialize_p1_variant_package_v5",
