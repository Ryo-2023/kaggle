...
def _manifest(prediction: Path, **overrides: Any) -> Path:
    ...
    payload: dict[str, Any] = {
        ...
        "d4_predictor_sha256_after": "f" * 64,
        "stage_predictor_sha256_after": "g" * 64,
...
def test_panel_stays_incomplete_until_all_five_samples_open(
    ...
    for sample_id in PANEL_V1:
        prediction = _prediction(root, sample_id)
        _manifest(prediction)
    inference = {
        "status": "READY",
        "selection_lock_id": lock["selection_lock_id"],
...
def test_aggregate_rejects_tampered_final_score() -> None:
...
def _sample_receipt(
    ...
    *,
    ...
    return {
        ...
        "prediction_manifest_path": f"predictions/{sample_id}.geff.manifest.json",
        "prediction_manifest_sha256": "a" * 64,
        "prediction_directory_sha256": "b" * 64,
        "prediction_node_count": 2,
        "prediction_edge_count": edge_tp + edge_fp,
        "prediction_fork_count": 0,
        "metric_config": {
...
def test_aggregate_requires_exact_five_and_uses_unweighted_macro() -> None:
    rows = [
        _sample_receipt(sample, 0.5 + index * 0.1, edge_weight=index + 1)
        for index, sample in enumerate(PANEL_V1)
    ]

    result = evaluation.aggregate_panel_receipts(rows, _lock())
    ...
    with pytest.raises(ValueError, match=r"PANEL_V1|five"):
        evaluation.aggregate_panel_receipts(rows[:-1], _lock())


def test_aggregate_preserves_division_null_and_live_semantics() -> None:
    rows = [_sample_receipt(sample, 0.9) for sample in PANEL_V1]
    no_division = evaluation.aggregate_panel_receipts(rows, _lock())
    assert no_division["panel_division_jaccard"] is None
    assert no_division["division_term_live"] is False

    rows[-1] = _sample_receipt(PANEL_V1[-1], 0.9, division=(0, 0, 1))
    with_division = evaluation.aggregate_panel_receipts(rows, _lock())
