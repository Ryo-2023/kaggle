...
6. **否定できた赤旗（再調査不要）:** 検出器固定は成立、メトリクスは上流とバイト一致、
...
### S2-2 キャッシュ書き出しが末尾で約 1.2 GB を要求する（12 時間ジョブの OOM リスク）

| 項目 | 内容 |
|---|---|
| 証拠 | `src/biohub/detector_fixed_race/cache.py:285-312`、`schema.py:214-251` |
| 実測 | 100 フレームキャッシュ manifest: `edge_count = 7,240,938`（監査前の独立推定 7,245,682、誤差 0.07%） |
| 確信度 | 中〜高（行あたりバイト数からの算術） |

`_build_edge_arrays` は各フレームペアの **n_src × n_tgt 全直積**を保持する。
11 配列の合計は 1 行あたり 66 B → **1 コピーで約 478 MB**。`write_detector_cache` は

1. `_write_npz(edges_path, ...)`（元コピー保持）
2. `_load_npz(edges_path)` で **2 つ目の完全コピー**を作る（`cache.py:310`）
3. `loaded_edges.validate(loaded_nodes)` が `expected_voxel_delta` / `expected_physical_delta`
   （各 87 MB）などの一時配列を作る（`schema.py:240-251`）

を行うため、**80 分の検出器実行が終わった最後**にピーク約 1.2 GB が乗る。
コンテナ上限 7.651 GiB、稼働中ジョブは既に約 4.2–4.34 GiB。パネル残り 3 試料でも同じ
ピークが再来する。落ちれば 80 分/試料が失われる。

同ファイルの旧版には `_build_edge_arrays` 内で 724 万要素の softmax を計算してから
すぐ上書きする無駄があった（`upstream_adapter.py:386-401`）。Codex の `f6f9ea3`
"Avoid redundant detector cache softmax pass" が既にこれに対処しているので、
残る指摘は上記の**二重コピー**のみ。

**推奨（低リスク）:** ラウンドトリップ検証を `_load_npz` の全体コピーではなく
`np.load(..., mmap_mode)` 相当か配列単位の逐次検証に変える。もしくは検証後に
`del loaded_edges` を明示して次の試料に入る前に解放する。
